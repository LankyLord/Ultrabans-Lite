            ...
            public void run() {
                ...
                if (bans.isEmpty()) {
                    ...
                    if (msg.contains(Ultrabans.AMOUNT))
                        msg = msg.replaceAll(Ultrabans.AMOUNT, String.valueOf(bans.size()));
                    if (msg.contains(Ultrabans.VICTIM))
                        msg = msg.replaceAll(Ultrabans.VICTIM, name);
                ...
                if (msg.contains(Ultrabans.AMOUNT))
                    msg = msg.replaceAll(Ultrabans.AMOUNT, String.valueOf(bans.size()));
                if (msg.contains(Ultrabans.VICTIM))
                    msg = msg.replaceAll(Ultrabans.VICTIM, bans.get(0).name);
                msg = Formatting.formatMessage(msg);
