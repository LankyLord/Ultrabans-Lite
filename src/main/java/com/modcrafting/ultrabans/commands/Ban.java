...
public class Ban implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (victim == null) {
            ...
            if (victim != null) {
                ...
                if (bcmsg.contains(Ultrabans.VICTIM))
                    bcmsg = bcmsg.replaceAll(Ultrabans.VICTIM, p.toLowerCase());
                plugin.bannedPlayers.add(p.toLowerCase());
                if (config.getBoolean("CleanOnBan"))
                    Formatting.deletePlyrdat(p);
        ...
        if (vicmsg.contains(Ultrabans.ADMIN))
            vicmsg = vicmsg.replaceAll(Ultrabans.ADMIN, admin);
        if (vicmsg.contains(Ultrabans.REASON))
            vicmsg = vicmsg.replaceAll(Ultrabans.REASON, reason);
        ...
        if (config.getBoolean("CleanOnBan", false))
            Formatting.deletePlyrdat(fname);
        if (config.getBoolean("ClearWarnOnBan", false))
            plugin.getUBDatabase().clearWarns(fname);
        if (broadcast)
            Bukkit.broadcastMessage(Formatting.formatMessage(bcmsg));
        else
            sender.sendMessage(ChatColor.ITALIC + "Silent: " + Formatting.formatMessage(bcmsg));
