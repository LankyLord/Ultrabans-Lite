  ...
  public boolean onCommand(final CommandSender sender, Command command, String label, final String[] args) {
    ...
    if (args[0].equals("*") && sender.hasPermission("ultrabans.kick.all")) {
      ...
      if (adminMsg.contains(Ultrabans.ADMIN)) {
        adminMsg = adminMsg.replaceAll(Ultrabans.ADMIN, admin);
      }
      if (adminMsg.contains(Ultrabans.REASON)) {
        adminMsg = adminMsg.replaceAll(Ultrabans.REASON, reason);
      }
    ...
    if (victim == null) {
      ...
      if (smvic.contains(Ultrabans.VICTIM)) {
        smvic = smvic.replaceAll(Ultrabans.VICTIM, p);
      }
